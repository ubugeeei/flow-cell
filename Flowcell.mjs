...
export function inspectGraph(scope) {
...
export function cell(initial, options) {
...
export const derived = createDerived;
...
export const asyncDerived = createAsyncDerived;
class ScopeImpl {
...
export function createScope(snapshot) {
  return new ScopeImpl(snapshot);
}
export function dehydrate(scope) {
...
export function preload(readable, scope) {
  const scopeImpl = asScopeImpl(scope);
  if (scopeImpl != null) {
...
export function keyed(factory, options) {
  const cache = new Map();
